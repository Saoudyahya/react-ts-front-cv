...
const App: React.FC = () => {
  ...
  const fetchCameraStatus = async () => {
    try {
      ...
      setIsCameraRunning(data.is_running);
    } catch (err) {
  ...
  const processFrame = async () => {
    ...
    try {
      ...
      setError('Error processing frame. Check if server is running.');
    } finally {
  ...
  useEffect(() => {
    const statusInterval = setInterval(fetchCameraStatus, 2000);
    ...
    <div className="app">
      ...
      <div className="container">
        ...
        <div className="panel camera-panel">
          ...
          {cameraStatus && isCameraRunning && (
            <div className="camera-info">
              <span>Backend: {cameraStatus.backend}</span>
              <span>Frames: {cameraStatus.frames_captured}</span>
              <span className={cameraStatus.has_frame ? 'status-ok' : 'status-error'}>
                {cameraStatus.has_frame ? '✓ Active' : '✗ No frames'}
              </span>
            </div>
          )}
